...
public class DocumentationTryClass {
    ...
    /**
     ...
     - Parameter test001p2: Paragraph about the other parameter that
                            emphatically *isn't* a string type on funny newline.
    ...
    /**
     Brief description using `brief` at-command, old-timey Java and Obj-C style.
     
     Longer description, and how, using the antedeluvial at-command
     `description` which appears to be a synonym for `details`. That's tested in
     another place. The description text is indented by three spaces, because
     there was some documentation commentary written like that in some test
     input in the early stages of development. The spaces get collapsed. There
     are also some artificially long words, that could be like verbose class or
     method names application(_:shouldAllowExtensionPointIdentifier:) and then
     this too disableThirdPartyKeyboard() which isn't as long. Check
